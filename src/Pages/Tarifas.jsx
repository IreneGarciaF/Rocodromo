...
function Tarifas() {
  ...
  try {
    const response = await fetch('https://git.heroku.com/rocodromo.git/create-checkout-session', {
      method: 'POST',
      headers: {
...
const handleCheckoutSubscription = async (priceId, name) => {
  ...
  try {
    ...
    const response = await fetch('https://git.heroku.com/rocodromo.git/create-checkout-session-subscription', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
